...
class TwoPlayerButton: SKSpriteNode {
    
    var selectHandler : ()->Void = { print("two player button not implemented! ")}
    
    
    init(){
        let texture = SKTexture(imageNamed: "twoPlayerButton")
        super.init(texture: texture, color: .clear, size: CGSize(width: 160, height: 60))
        self.zPosition = 1
        self.alpha = 0
        self.position = CGPoint(x: 333, y: 221)
